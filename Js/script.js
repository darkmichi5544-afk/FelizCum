...
function showDedicationText() { //seguidores
  ...
  if (!text) {
    text = `Para Mi cumpleañera del día:\n\nMuchas Felicidades por tus 23 añitos Corazom🖤. Desde que te vi super que serias la personita a la que entregaria todo de mi. Graciar por compartir conmigo Tu sonrisa, tu voz, tu forma de ser… todo en ti me hace sentir en un espacio más feliz .\n\nDe ti que he podido aprender muchas más cosas y encontrar el razonamiento de que se es mejor siendo feliz que pensar en un "que diran".\n\nTe quiero mucho más de lo que mis palabras puedan expresar.`;  } else {
...
function showSignature() {
  ...
  signature.textContent = firma ? decodeURIComponent(firma) : "Con amor, Eitan pesado -.-";
...
function showCountdown() {
  ...
  // Se mantiene eventParam y eventDate por si se usan en otro lugar, pero no se usarán para mostrar el conteo.
  let eventParam = getURLParam('event'); 
  let startDate = startParam ? new Date(startParam + 'T00:00:00') : new Date('2025-06-21T00:00:00'); 
  let eventDate = eventParam ? new Date(eventParam + 'T00:00:00') : new Date('2026-10-29T00:00:00');

  function update() {
    const now = new Date();
    // Cálculo de días transcurridos desde startDate
    let diff = now - startDate;
    let days = Math.floor(diff / (1000 * 60 * 60 * 24));
    
    // El resto de las variables de tiempo del evento no se necesitan para la salida, pero se mantienen
    // para que la función sea mínima la modificación.
    ...
    // *** MODIFICACIÓN CLAVE: Solo se incluye la línea de "días transcurridos" ***
    container.innerHTML =
      `Desde que nos conocimos han pasado: <b>${days}</b> días`;
